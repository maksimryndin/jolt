...
impl<const WORD_SIZE: usize> VirtualInstructionSequence<WORD_SIZE> for DIVUInstruction<WORD_SIZE> {
    const SEQUENCE_LENGTH: usize = 9;
    
    fn virtual_trace(trace_row: RVTraceRow<WORD_SIZE>) -> Vec<RVTraceRow<WORD_SIZE>> {
        assert_eq!(trace_row.instruction.opcode, RV_IM::DIVU);
        ...
        virtual_trace.push(RVTraceRow {
            instruction: ELFInstruction {
                address: trace_row.instruction.address,
                opcode: RV_IM::VIRTUAL_ASSERT_VALID_UNSIGNED_REMAINDER,
                rs1: v_r,
                rs2: r_y,
                rd: None,
                imm: None,
        ...
        virtual_trace.push(RVTraceRow {
            instruction: ELFInstruction {
                address: trace_row.instruction.address,
                opcode: RV_IM::VIRTUAL_ASSERT_LTE,
                rs1: v_qy,
                rs2: r_x,
                rd: None,
                imm: None,
...
mod test {
    ...
    fn divu_virtual_sequence_32() {
        jolt_virtual_sequence_test!(DIVUInstruction::<32>, RV_IM::DIVU);
    }
}
