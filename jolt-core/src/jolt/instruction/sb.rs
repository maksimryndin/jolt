...
mod test {
    ...
    fn sb_virtual_sequence_32() {
        ...
        for _ in 0..256 {
            ...
            let byte = rs2_val & 0xff;

            let word_address = (dest >> 2) << 2;
            let word_before = rng.next_u32() as u64;

            let word_after = match dest % 4 {
                0 => byte | (word_before & 0xffffff00),
                1 => (byte << 8) | (word_before & 0xffff00ff),
                2 => (byte << 16) | (word_before & 0xff00ffff),
                3 => (byte << 24) | (word_before & 0x00ffffff),
                _ => unreachable!(),
            };

            let sb_trace_row = RVTraceRow {
                instruction: ELFInstruction {
