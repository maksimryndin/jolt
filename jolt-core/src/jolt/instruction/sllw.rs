...
pub struct SLLWInstruction(pub u64, pub u64);

impl SLLWInstruction {
    const WORD_SIZE: usize = 64;
}

impl JoltInstruction for SLLWInstruction {
    ...
    fn subtables<F: JoltField>(
        ...
        let mut subtables: Vec<Box<dyn LassoSubtable<F>>> = vec![
            Box::new(SllSubtable::<F, 0, 32>::new()),
            Box::new(SllSubtable::<F, 1, 32>::new()),
            Box::new(SllSubtable::<F, 2, 32>::new()),
            Box::new(SllSubtable::<F, 3, 32>::new()),
        ];
    ...
    fn lookup_entry(&self) -> u64 {
        ...
        (self.0 as u32)
            .checked_shl(self.1 as u32 % 32 as u32)
            .unwrap_or(0)
            .into()
